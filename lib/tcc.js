...
// Public interfacce to change Fan settings

tcc.prototype.ChangeThermostatFan = function(desiredState) {
  // debug("ChangeThermostat()", desiredState);
  return queue.add(async () => {
    try {
      if (!this.sessionID) {
      ...
      var CommTaskID = await this._UpdateThermostatFan(desiredState);
      await this._GetCommTaskState(CommTaskID);
      var thermostat = await this._GetThermostat(desiredState.ThermostatID);
      return (thermostat);
    } catch (err) {
...
tcc.prototype._UpdateThermostatFan = function(desiredState) {
  debug("_UpdateThermostatFan()", desiredState);
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        HEADER.soapAction = 'http://services.alarmnet.com/Services/MobileV2/ChangeThermostatFan';
        var message = '<?xml version="1.0" encoding="utf-8"?>' + parser.toXml(tccMessage.soapMessage(tccMessage.ChangeThermostatFanMessage(this.sessionID, desiredState)));
        // debug("_UpdateThermostatFan: SOAP Message", message, this.sessionID, desiredState);
        var {
          response
        } = await soapRequest({
          url: URL,
        ...
        if (response.statusCode === 200) {
          /*
          debug("_UpdateThermostatFan", JSON.stringify(parser.toJson(response.body, {
            object: true,
            reversible: false,
            coerce: true,
            sanitize: false,
          ...
          */
          var ChangeThermostatFan = parser.toJson(response.body, {
            object: true,
            reversible: false,
            coerce: true,
            sanitize: false,
            ...
            arrayNotation: false,
            alternateTextNode: false
          })["soap:Envelope"]["soap:Body"].ChangeThermostatFanResponse.ChangeThermostatFanResult;
          // debug("_UpdateThermostatFan", ChangeThermostatFan);
          if (ChangeThermostatFan.Result === "Success") {
            debug("Success: _UpdateThermostatFan %s", ChangeThermostatFan, message);
            resolve(ChangeThermostatFan.CommTaskID);
          } else {
            this.sessionID = null;
            debug("ERROR: _UpdateThermostatFan %s", ChangeThermostatFan.Result, message);
            reject(new Error("ERROR: _UpdateThermostatFan %s", ChangeThermostatFan.Result));
          }
        } else {
          debug("ERROR: _UpdateThermostatFan %s", response, message);
          reject(new Error("ERROR: _UpdateThermostatFan %s", ChangeThermostatFan.Result));
        }
      } catch (err) {
        // console.error("_UpdateThermostatFan Error:", err);
        debug("ERROR: _UpdateThermostatFan message", parser.toXml(tccMessage.soapMessage(tccMessage.ChangeThermostatFanMessage(this.sessionID, desiredState))));
        reject(err);
        this.sessionID = null;
      }
    })();
