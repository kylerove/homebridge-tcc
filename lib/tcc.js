...
tcc.prototype.ChangeThermostatFan = function(desiredState) {
  debug("ChangeThermostatFan()", desiredState);
  return queue.add(async () => {
    try {
      if (!this.sessionID) {
        this.sessionID = await this._login();
      ...
      debug("ChangeThermostatFan: SOAP Message", message, this.sessionID, desiredState);
      var {
        response
      } = await soapRequest({
        url: URL,
      ...
      if (response.statusCode === 200) {
        debug("ChangeThermostatFan", parser.toJson(response.body, {
          object: true,
          reversible: false,
          coerce: true,
          sanitize: false,
        ...
        debug("ChangeThermostatFan", ChangeThermostatFan);
        if (ChangeThermostatFan.Result === "Success") {
          debug("Success: _UpdateThermostat %s", ChangeThermostatFan, message);
          return (ChangeThermostatFan.CommTaskID);
        } else {
          this.sessionID = null;
          debug("ERROR: ChangeThermostatFan %s", ChangeThermostatFan.Result, message);
          return (new Error("ERROR: ChangeThermostatFan %s", ChangeThermostatFan.Result));
        }
      } else {
        debug("ERROR: ChangeThermostatFan %s", response, message);
        throw new Error("ERROR: ChangeThermostatFan %s", ChangeThermostatFan.Result);
      }
    } catch (err) {
      // console.error("_UpdateThermostat Error:", err);
      debug("ChangeThermostatFan message", parser.toXml(tccMessage.soapMessage(tccMessage.ChangeThermostatFanMessage(this.sessionID, desiredState))));
