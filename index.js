...
function tccPlatform(log, config, api) {
  ...
  this.humidifier = config['humidifier'];
...
tccPlatform.prototype.didFinishLaunching = function() {
  ...
  thermostats.pollThermostat().then((devices) => {
    for (var zone in devices.hb) {
      ...
      if ((this.sensors == "all" || this.sensors == "outside") && outsideSensors == 0 && Number.isFinite(devices.hb[zone].OutsideTemperature) === true && Number.isFinite(devices.hb[zone].OutsideHumidity) === true && ) {
      ...
      // does user want separate humidifier accessory? if so, create one
      if (this.humidifier === true && humidifier == 0 && (devices.hb[zone].Humidification.CanControlHumidification === true || devices.hb[zone].Humidification.CanControlDehumidification === true)) {
        var newHumidiferAccessory = new TccHumidiferAccessory(this, devices.hb[zone], this.humidifier);
        humidifier = 1;
      }
      else if (this.humidifier === true && (devices.hb[zone].Humidification.CanControlHumidification === false && devices.hb[zone].Humidification.CanControlDehumidification === false)) {
        this.log("Error - Honeywell reports that your system does not support humidication or dehumidification control. Please set 'humidifier' to 'false' in the plugin configuration.");
      }
    }
  }).catch((err) => {
...
tccPlatform.prototype.configureAccessory = function(accessory) {
  ...
  // humidifier
  if (accessory.getService(Service.HumidifierDehumidifier)) {
    accessory.log = this.log;
    
    accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic.Active)
      .on('set', setHumidifyingDehumidifyingActive.bind(accessory));

    accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic.TargetHumidifierDehumidifierState)
      .on('set', setTargetHumidifyingDehumidifying.bind(accessory));

    accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic.RelativeHumidityHumidifierThreshold)
      .on('set', setHumidityHumidifierThreshold.bind(accessory));
    
    accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic..RelativeHumidityDehumidifierThreshold)
      .on('set', setHumidityDehumidifierThreshold.bind(accessory));
    
    // only attach this to the humidifier accessory, not the sensors accessory
    accessory.context.ChangeHumidifier = new ChangeHumidifier(accessory);
    debug("configureAccessory", accessory.context.ChangeHumidifier);
  }
  ...
  thermostats.pollThermostat().then((devices) => {
    myAccessories.forEach(function(accessory) {
      ...
      if (devices.hb[accessory.context.ThermostatID]) {
        ...
        if (accessory.getService(Service.HumidifierDehumidifier)) {
          accessory.getService(Service.HumidifierDehumidifier).getCharacteristic(Characteristic.RelativeHumidityHumidifierThreshold)
            .updateValue(new Error("Status missing for thermostat"));
          }
        if (accessory.getService(Service.HumidifierDehumidifier)) {
          accessory.getService(Service.HumidifierDehumidifier).getCharacteristic(Characteristic.RelativeHumidityDehumidifierThreshold)
            .updateValue(new Error("Status missing for thermostat"));
          }
      }
    }.bind(this));
...
function updateStatus(accessory, device) {
  ...
  // update humidifier
  if (accessory.getService(device.name + " Humidifier")) {
    var service = accessory.getService(Service.HumidifierDehumidifier);
    
    service.getCharacteristic(Characteristic.Active)
      .updateValue(device.HumidifierDehumidifierActive);
    service.getCharacteristic(Characteristic.CurrentHumidifierDehumidifierState)
      .updateValue(device.CurrentHumidifyingDehumidifyingState);
    service.getCharacteristic(Characteristic.TargetHumidifierDehumidifierState)
      .updateValue(device.TargetHumidifyingDehumidifyingState);
    service.getCharacteristic(Characteristic.CurrentRelativeHumidity)
      .updateValue(device.InsideHumidity);
    service.getCharacteristic(Characteristic.RelativeHumidityDehumidifierThreshold)
      .updateValue(device.TargetDehumidifierThreshold);
    service.getCharacteristic(Characteristic.RelativeHumidityHumidifierThreshold)
      .updateValue(device.TargetHumidifierThreshold);
  }
}

function TccHumidiferAccessory(that, device, humidifier) {
  this.log = that.log;
  //this.log("Adding TCC Device", device.Name);
  this.name = device.Name + " Humidifier";
  this.ThermostatID = device.ThermostatID;
  this.device = device;
  this.storage = that.storage;
  this.refresh = that.refresh;
  debug("TccHumidiferAccessory()",device);

  var uuid = UUIDGen.generate(this.name + " - TCC");

  if (!getAccessoryByName(this.name)) {
    this.log("Adding TCC Humidifier Device (deviceID="+this.ThermostatID+")", this.name);
    ...
    this.accessory.getService(Service.AccessoryInformation)
      ...
      .setCharacteristic(Characteristic.Model, device.Model + "-Attached Humidifier")
      .setCharacteristic(Characteristic.SerialNumber, hostname + "-" + this.name)
      .setCharacteristic(Characteristic.FirmwareRevision, require('./package.json').version);

    this.accessory.addService(Service.HumidifierDehumidifier, this.name);
    ...
    this.accessory
      ...
      .getCharacteristic(Characteristic.TargetHumidifierDehumidifierState)
      .setProps({
        validValues: device.TargetHumidifyingDehumidifyingStateValidValues
      })
      .on('set', setTargetHumidifyingDehumidifying.bind(this.accessory));
    ...
    this.accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic.RelativeHumidityDehumidifierThreshold)
      .on('set', setHumidityDehumidifierThreshold.bind(this.accessory));
      
    this.accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic.RelativeHumidityHumidifierThreshold)
      .on('set', setHumidityHumidifierThreshold.bind(this.accessory));

    this.accessory
      .getService(Service.HumidifierDehumidifier)
      .getCharacteristic(Characteristic.Active)
      .on('set', setHumidifyingDehumidifyingActive.bind(this.accessory));

    this.accessory
      .getService(Service.Thermostat).log = this.log;

    this.accessory.context.ChangeHumidifier = new ChangeHumidifier(this.accessory);
    that.api.registerPlatformAccessories("homebridge-tcc", "tcc", [this.accessory]);
    myAccessories.push(this.accessory);
    return this.accessory;
  } else {
    this.log("Existing TCC humidifier accessory (deviceID="+this.ThermostatID+")", this.name);

    // need to check if accessory/zone/thermostat already exists, but user added humidifier in config then must declare
...
function setHumidityHumidifierThreshold(value, callback) {
  this.log("Setting target humidity for", this.displayName, "to", value + "%");
  this.context.logEventCounter = 9;
  // debug("this", this);
  this.context.ChangeHumidifier.put({
    TargetHumidifierThreshold: value
  }).then((thermostat) => {
    // debug("setHumidityHumidifierThreshold", this, thermostat);
    updateStatus(this, thermostat);
    callback(null);
  }).catch((error) => {
    callback(error);
...
function setHumidityDehumidifierThreshold(value, callback) {
  this.log("Setting target humidity for", this.displayName, "to", value + "%");
  this.context.logEventCounter = 9;
  // debug("this", this);
  this.context.ChangeHumidifier.put({
    TargetDehumidifierThreshold: value
  }).then((thermostat) => {
    // debug("setHumidityDehumidifierThreshold", this, thermostat);
    updateStatus(this, thermostat);
    callback(null);
  }).catch((error) => {
    callback(error);
...
function setHumidifyingDehumidifyingActive(value, callback) {
  this.log("Setting switch for", this.displayName, "to", value);
  this.context.logEventCounter = 9;
  this.context.ChangeHumidifier.put({
    HumidifierDehumidifierActive: value
  }).then((thermostat) => {
    // debug("setHumidifyingDehumidifyingActive", this, thermostat);
    updateStatus(this, thermostat);
    callback(null);
  }).catch((error) => {
    callback(error);
...
function setTargetHumidifyingDehumidifying(value, callback) {
  this.log("Setting switch for", this.displayName, "to", value);
  this.context.logEventCounter = 9;
  this.context.ChangeHumidifier.put({
    TargetHumidifyingDehumidifyingState: value
  }).then((thermostat) => {
    // debug("setTargetHumidifyingDehumidifying", this, thermostat);
    updateStatus(this, thermostat);
    callback(null);
  }).catch((error) => {
    callback(error);
...
function setHeatingThresholdTemperature(value, callback) {
  ...
  this.context.ChangeThermostat.put({
    ...
    // debug("setHeatingThresholdTemperature", this, thermostat);
    updateStatus(this, thermostat);
    callback(null);
  }).catch((error) => {
    callback(error);
...
function setCoolingThresholdTemperature(value, callback) {
  ...
  this.context.ChangeThermostat.put({
    ...
    // debug("setCoolingThresholdTemperature", this, thermostat);
    updateStatus(this, thermostat);
    callback(null);
  }).catch((error) => {
    callback(error);
...
function ChangeHumidifier(accessory) {
  // debug("ChangeThermostat", accessory);
  this.desiredState = {};
  this.deferrals = [];
  this.ThermostatID = accessory.context.ThermostatID;
...
ChangeHumidifier.prototype.put = function(state) {
  debug("put %s ->", this.ThermostatID, state);
  return new Promise((resolve, reject) => {
    this.desiredState.ThermostatID = this.ThermostatID;
    for (const key in state) {
    ...
    if (!this.timeout) {
      this.timeout = setTimeout(() => {
        // debug("put start");
        thermostats.ChangeHumidifier(this.desiredState).then((thermostat) => {
          for (const d of this.deferrals) {
            d.resolve(thermostat);
          }
          this.desiredState = {};
